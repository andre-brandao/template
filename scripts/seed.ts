...
const verbs = ["Review", "Create", "Update", "Plan", "Ship", "Polish", "Document", "Test", "Refactor", "Explore"]
const nouns = ["dashboard", "auth flow", "todo list", "insights", "settings", "forms", "layout", "data model", "API", "deploy"]
const scopes = ["for launch", "with the team", "before Friday", "after feedback", "for demo", "in staging", "for users"]
...
const result = await Database.provide(url, async () => {
  ...
  const auth = user ? await Auth.login({ email, password }) : await Auth.register({ name, email, password })
  ...
  await Actor.provide("user", { userID: auth.userID }, async () => {
    ...
    await Promise.all(
      Array.from({ length: count }, async (_, i) => {
        ...
        const status = i >= 11 && i < 16 ? (i % 2 === 0 ? "pending" : "in_progress") : pick(statuses)
